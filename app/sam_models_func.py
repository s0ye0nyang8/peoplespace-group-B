...
def predict(x, learn: Learner):
    ...
    path_test = Path(x)
    #폴더 내의 파일 개수 구하기
    image_count = len(os.walk(x).__next__()[2])
    ...
    .databunch(bs=image_count, collate_fn=bb_pad_collate))
    ...
    with torch.no_grad():
        ...
        for img, clas_pred, bbox_pred in list(
            ...
            bbox, label = show_results_predict(img, bbox_pred, preds, scores, learn.data.train_ds.classes[1:] , (5, 5), titleB="Prediction")
