        ...
        childData.brddate = dateFormat(childData.brddate,"yyyy-mm-dd");
          ...
          .then((snapshot) => {
              ...
              snapshot.forEach((doc) => {
                 ...
                 var childData = doc.data();
                 childData.brddate = dateFormat(childData.brddate, "yyyy-mm-dd");
                 if ( childData.start!=null &&start!=null&& childData.start.toString() == start.toString() && now.getTime() <= childData.end.toMillis() ){    // 지금. 현재. 진행중인 수업만 차트로 나타낼거니까!
                    console.log("###### "+rows);
                     rows.push(childData);
                 }
  ...
  if (!fb.auth().currentUser) {
    ...
    .orderBy("start", "desc").get() //where orderby 같이 쓰려면 색인 추가해줘얗.ㅁ 
      .then((snapshot) => {
          ...
          res.render('history', {rows: rows, ids: ids, teacher:fb.auth().currentUser.email});
          ...
          //   });
          //   res.render('history', {rows: rows, ids: ids, subs:subs, teacher:fb.auth().currentUser.email});
          //   console.log("히스토리");
          //   console.log(rows, subs, ids, teacher);
          // })
          // .catch((err) => {
...
router.get('/byClass', function(req, res, next) {
  ...
  .get().then((snapshot) =>{
    ...
    /* 정보를 가져갈 페이지로 각 배열(row)을 보내나 */
    res.render('byClass', {rows: rows, ids: ids, teacher:fb.auth().currentUser.email});
  })
  .catch((err) => {
      console.log('Error getting documents: history', err);
      process.exit()
...
router.post('/classroom', function(req, res, next) {
  ...
  .get()
      .then((snapshot) => {
          ...
          /* 정보를 가져갈 페이지로 각 배열(row)을 보내나 */
          res.render('classroom', {rows: rows, ids: ids, attentions:attentions, subject: post['subject']});
      })
      .catch((err) => {
          console.log('Error getting documents', err);
      });
...
// router.post('/submit_category', function(req, res, next) {
//   if (!fb.auth().currentUser) {
//     res.redirect('fb');
//     return;
//   }
